...
namespace CGD.UI
{
    public class BoardPanel : MenuPanel
    {
        ...
        public void Awake()
        {
            NetworkEvents.OnNextPlayersTurn += NextPlayersTurn;
            NetworkEvents.OnPlayerSkippedTurn += SkippedTurnPrompt;
            NetworkEvents.OnPlayerSubmittedClue += AnimateCardSubmission;
            /*
        ...
        private void DrawBoardPlayers()
        {
            ...
            foreach (var boardPlayer in boardPlayers)
            {
                ...
                else
                {
                    ...
                    byte teamByte = team == null ? (byte)0 : team.Code;
                    int icon = 0;

                    if (player.CustomProperties.TryGetValue(PlayerProperties.Icon, out object Icon))
                        icon = (int)Icon;

                    boardPlayer.gameObject.SetActive(true);
                    boardPlayer.Draw(player.NickName, icon, player.ActorNumber, teamByte);
                }
            }
        }
        ...
        private void NextPlayersTurn(int actorNumber, double networkTime)
        {
