...
SubShader {
    ...
    Pass {
	   ...
	   uniform float4 _SpotLightDirection;
	   uniform float4 _SpotLightPosition;
	   uniform float _SpotLightAngle;
	   uniform float _StrengthScalor;//
       uniform float _SpotLightRange;
       uniform float _SpotLightIntensity;
        ...
        float3 toFragment = normalize(worldPos - _SpotLightPosition.xyz); 
        float3 lightDir = normalize(_SpotLightDirection.xyz);
        float distanceToFragment = length(input.worldPos - _SpotLightPosition.xyz);

        // Calculate the dot product to determine if the fragment is within the spotlight cone
